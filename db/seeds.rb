...
w1.new_game(seahawks, packers)
w1.new_game(chiefs, patriots)
w1.new_game(jets, bills)
w1.new_game(falcons, bears)
w1.new_game(ravens, bengals)
w1.new_game(steelers, browns)
w1.new_game(cardinals, lions)
w1.new_game(jaguars, texans)
w1.new_game(raiders, titans)
w1.new_game(eagles, redskins)
w1.new_game(colts, rams)
w1.new_game(panthers, sanfran)
w1.new_game(giants, cowboys)
w1.new_game(saints, vikings)
w1.new_game(chargers, broncos)
...
w2.new_game(cardinals, colts)
w2.new_game(titans, jaguars)
w2.new_game(eagles, chiefs)
w2.new_game(patriots, saints)
w2.new_game(vikings, steelers)
w2.new_game(bears, bucs)
w2.new_game(dolphins, chargers)
w2.new_game(jets, raiders)
w2.new_game(cowboys, broncos)
w2.new_game(redskins, rams)
w2.new_game(sanfran, seahawks)
w2.new_game(lions, giants)


w3 = Week.create(week_number: 3)

w3.new_game(bengals, packers)
w3.new_game(rams, sanfran)
w3.new_game(ravens, jaguars)
w3.new_game(broncos, bills)
w3.new_game(saints, panthers)
w3.new_game(steelers, bears)
w3.new_game(falcons, lions)
w3.new_game(browns, colts)
w3.new_game(bucs, vikings)
w3.new_game(texans, patriots)
w3.new_game(dolphins, jets)
w3.new_game(giants, eagles)
w3.new_game(seahawks, titans)
w3.new_game(chiefs, chargers)
w3.new_game(raiders, redskins)
w3.new_game(cowboys, cardinals)

w4 = Week.create(week_number: 4)

w4.new_game(bears, packers)
w4.new_game(saints, dolphins)
w4.new_game(bills, falcons)
w4.new_game(steelers, ravens)
w4.new_game(bengals, browns)
w4.new_game(rams, cowboys)
w4.new_game(titans, texans)
w4.new_game(lions, vikings)
w4.new_game(panthers, patriots)
w4.new_game(jaguars, jets)
w4.new_game(sanfran, cardinals)
w4.new_game(eagles, chargers)
w4.new_game(giants, bucs)
w4.new_game(raiders, broncos)
w4.new_game(colts, seahawks)
w4.new_game(redskins, chiefs)

w5 = Week.create(week_number: 5)

w5.new_game(packers, cowboys)
w5.new_game(patriots, bucs)
w5.new_game(bills, bengals)
w5.new_game(jets, browns)
w5.new_game(panthers, lions)
w5.new_game(sanfran, colts)
w5.new_game(titans, dolphins)
w5.new_game(chargers, giants)
w5.new_game(cardinals, eagles)
w5.new_game(jaguars, steelers)
w5.new_game(seahawks, rams)
w5.new_game(ravens, raiders)
w5.new_game(chiefs, texans)
w5.new_game(vikings, bears)

w6 = Week.create(week_number: 6)

w6.new_game(packers, vikings)
w6.new_game(eagles, panthers)
w6.new_game(dolphins, falcons)
w6.new_game(bears, ravens)
w6.new_game(browns, texans)
w6.new_game(lions, saints)
w6.new_game(patriots, jets)
w6.new_game(sanfran, redskins)
w6.new_game(bucs, cardinals)
w6.new_game(rams, jaguars)
w6.new_game(chargers, raiders)
w6.new_game(steelers, chiefs)
w6.new_game(giants, broncos)
w6.new_game(colts, titans)

w7 = Week.create(week_number: 7)

w7.new_game(saints, packers)
w7.new_game(chiefs, raiders)
w7.new_game(bucs, bills)
w7.new_game(panthers, bears)
w7.new_game(titans, browns)
w7.new_game(jaguars, colts)
w7.new_game(cardinals, rams)
w7.new_game(jets, dolphins)
w7.new_game(ravens, vikings)
w7.new_game(cowboys, sanfran)
w7.new_game(bengals, steelers)
w7.new_game(broncos, chargers)
w7.new_game(seahawks, giants)
w7.new_game(falcons, patriots)
w7.new_game(redskins, eagles)
